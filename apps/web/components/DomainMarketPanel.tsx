...
import { useBuyDomain, useMakeOffer } from "../lib/hooks/useMarketplaceActions";
      ...
      <Card className="glass-dark md:col-span-1">
        ...
        <CardContent className="space-y-2 text-xs">
          ...
          <div className="pt-3 flex gap-2 items-center">
            ...
            />
            <Button size="sm" disabled={!actionPrice || buyMut.isPending} onClick={() => buyMut.mutate({ domain: lower, price: actionPrice, currency: 'ETH' })}>Buy</Button>
            <Button size="sm" variant="outline" disabled={!actionPrice || offerMut.isPending} onClick={() => offerMut.mutate({ domain: lower, price: actionPrice, currency: 'ETH' })}>Offer</Button>
          </div>
        ...
        <CardContent className="max-h-64 overflow-auto text-xs space-y-1">
          {listingsQ.data.slice(0,10).map(l => (
            <div key={l.id} className="flex justify-between">
              ...
              <span className="truncate max-w-[120px]" title={l.seller}>{l.seller}</span>
            </div>
          ))}
          ...
          <div className="space-y-1">
            ...
            {offersQ.data.slice(0,10).map(o => (
              <div key={o.id} className="flex justify-between">
                ...
                <span className="truncate max-w-[120px]" title={o.buyer}>{o.buyer}</span>
              </div>
            ))}

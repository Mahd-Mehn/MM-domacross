...
export default function ConsolidatedMarketplacePage() {
  ...
  const fetchDomains = async () => {
    try {
      // Try to fetch from our enhanced marketplace API first
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data)) {
          // New marketplace API format
          const convertedDomains = data.map((listing: any) => ({
            name: `domain_${listing.tokenId}`,
            tld: 'eth',
            price: listing.price,
            offers_count: 0,
            listings_count: 1,
          ...
          setFilteredDomains(convertedDomains);
        } else {
          // Original API format

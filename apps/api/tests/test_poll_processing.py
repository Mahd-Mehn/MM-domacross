...
def test_process_name_token_purchased_updates_trades_and_portfolio():
    ...
    # Arrange users and competition/participants with randomized addresses to avoid uniqueness conflicts
    buyer_addr = f"0xaaa1111111111111111111111111{secrets.token_hex(4)}"[:42]
    seller_addr = f"0xbbb2222222222222222222222222{secrets.token_hex(4)}"[:42]
    ...
    comp = Competition(
        contract_address=f"0xcccc3333333333333333333333{secrets.token_hex(4)}"[:42],
        chain_id=1234,
        name="Test Comp",
        description="",
    ...
    unique_id = f"evt-{secrets.token_hex(6)}"
    price = "100.50"
    events = [
        {
            "id": 1,
            ...
            "txHash": f"0xdead{secrets.token_hex(8)}",
            "eventData": {
                "tokenId": "42",
                "tokenAddress": f"0xdddd4444444444444444444444{secrets.token_hex(4)}"[:42],
                "buyer": buyer_addr,
                "seller": seller_addr,
                "payment": {"price": price},
            },
